...
extension Feed {
    ...
    public func unfollow(fromTarget target: FeedId, keepHistory: Bool = false, completion: @escaping StatusCodeCompletion) -> Cancellable {
        return Client.feedSharedClient.request(endpoint: FeedEndpoint.unfollow(feedId,
                                                                     target: target,
    ...
    public func followers(offset: Int = 0, limit: Int = 25, completion: @escaping FollowersCompletion) -> Cancellable {
        ...
        return Client.feedSharedClient.request(endpoint: FeedEndpoint.followers(feedId, offset: offset, limit: limit)) { [weak self] result in
            if let self = self {
    ...
    public func following(filter: FeedIds = [],
        ...
        return Client.feedSharedClient.request(endpoint: FeedEndpoint.following(feedId,
                                                                      filter: filter,
                                                                      offset: offset,
